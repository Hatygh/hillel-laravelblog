    ...
    <form method="post" enctype="multipart/form-data" action="{{ route('posts.update', $post) }}">
            ...
            <div class="col-sm-7">
                <select  class="custom-select" name="user_id"
                    ...
                    @foreach($users as $user)
                        <option
                            @if ($user->id == @old('user_id', $post->user->id))
                            selected
                            @endif
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-7">
                <input type="text"
                       ...
                       @else class="form-control is-valid"
                       @endif
                       name="title" placeholder="Title" value="{{ @old('title', $post->title) }}">
            </div>
            @if ($errors->has('title'))
                <div class="col-sm-3">
        ...
        <div class="form-group row">
            ...
            <textarea class = "form-control" name="preview_text" rows="3"
                      ...
                      @else class="form-control is-valid"
                   @endif>{{ @old('preview_text', $post->preview_text) }} </textarea>
        ...
        <div class="form-group row">
            ...
            <textarea class = "form-control" name="body" rows="10"
                      ...
                      @else class="form-control is-valid"
                   @endif>{{ @old('body', $post->body) }} </textarea>
            @if ($errors->has('body'))
                <div class="col-sm-3">
